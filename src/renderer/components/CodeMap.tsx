...
export function CodeMap({ mode = 'full' }: { mode?: 'full' | 'display' }): JSX.Element | null {
  ...
  return (
    <div className={`flex flex-col min-h-0 ${isDisplay ? 'w-full' : 'h-full w-full'} bg-surface-1`} data-testid="code-map-dock">
      ...
      {/* Comprehension Health — the north-star metric: how much of the changed
          system you still understand. Decays as AI writes and you don't look. */}
      {health && health.liveModules > 0 && (
        <ComprehensionHealthBar health={health} drillAccuracy={drillAccuracy} onPick={setQuery} />
      )}

      {/* Governance: measurement → action. A workspace accumulating unconfirmed
      ...
      {governance.action !== 'ok' && (
        ...
        >
          ...
          <span className="text-text-secondary leading-snug">
            {governance.reason}
            {governance.modules.length > 0 && (
              <span className="text-text-muted">（{governance.modules.join('、')}）</span>
            )}
          </span>
        </div>
      )}

      {/* Ask the Map: type a module → highlight (dock mode only; sidebar uses main chat) */}
      {!isDisplay && <div className="px-3 py-1.5 border-b border-border">
        <input
          value={query}
        ...
        {brain && (
          <ModuleDossier
            brain={brain}
            annotations={annotationsForModule(pluginAnnotations, brain.focus)}
            onOpenFile={openFile}
            onSendToChat={sendToChat}
            onPick={setQuery}
            onCommitLens={commitLens}
          />
        )}
      </div>}

      {/* ── Comprehension chat (dock mode) or compact feed (sidebar mode) ── */}
      {!isDisplay && map && (
        <BrainChat
          data={{ map, decisions, commits, changes }}
          onOpenFile={openFile}
          onPick={setQuery}
          narrativeInput={{ changes, commits, decisions, lastSeen, protectedGlobs }}
        />
      )}
      {isDisplay && (
        <SidebarFeed
      ...
      {/* Comprehension Replay: scrub the timeline, watch the structure evolve. */}
      <ReplayBar steps={buildReplay({ changes, commits })} onScrub={setReplayIds} />

      {/* ── Advanced tools (collapsed by default to reduce cognitive load) ── */}
      <AdvancedToolsGroup>
        {/* Understand this system in 15 min — guided tour over the Living Map. */}
        {map && (
          <OnboardingTour
            steps={buildOnboardingTour({
              map,
              protectedGlobs,
              debtModules: debt?.modules ?? [],
              coupling: couplingEdges,
        ...
        {/* Comprehension-ranked diff — read the lines that matter, hide the churn. */}
        <RankedDiffPanel workspacePath={workspacePath} onOpenFile={openFile} />

        {/* Comprehension drill — measure (and build) your model of the system. */}
        {map && (
          <DrillPanel
            map={map}
            engaged={new Set(decisions.flatMap((d) => d.modules))}
            workspacePath={workspacePath}
            pluginDrills={pluginDrills}
            onFocus={setQuery}
            onAccuracy={setDrillAccuracy}
          />
        )}

        {/* Predict a task's blast radius BEFORE dispatch (autopsy → flight plan). */}
        {map && (
          <PreflightPredict
            data={{ map, protectedGlobs, debtModules: debt?.modules ?? [], couplingEdges }}
            onPredict={setPredictIds}
            onPick={setQuery}
          />
        )}

        {/* Cross-repo service map: link services that share events/HTTP routes. */}
      ...
      <div className="flex items-center gap-3 px-3 py-1.5 border-b border-border text-xs text-text-muted">
        ...
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full border border-dashed" style={{ borderColor: 'var(--color-danger)' }} /> contract</span>
        <span className="flex items-center gap-1"><span className="inline-block w-3 border-t-2 border-dashed" style={{ borderColor: 'var(--color-warning)' }} /> dep shift</span>
        <span className="flex items-center gap-1"><span className="inline-block w-3 border-t-2 border-dotted" style={{ borderColor: 'var(--color-purple, #a78bfa)' }} /> hidden coupling</span>
      </div>
      ...
      {track && track.total > 0 && (
        ...
        >
          <span className="text-text-secondary font-medium">Track Record</span>
          <span>{track.total} 次变更</span>
          <span title="自动放行 / 需审比例">
            auto <span className="text-text-secondary">{pct(track.autoRate)}</span>
          </span>
          <span title="跑了测试的变更占比">
            验证 <span className={track.verifyRate < 0.5 ? 'text-warning' : 'text-text-secondary'}>{pct(track.verifyRate)}</span>
          </span>
          {track.acceptRate !== undefined && (
            <span title="人在闸门处直接通过(未要求修改)的比例">
              采纳 <span className="text-text-secondary">{pct(track.acceptRate)}</span>
            </span>
          )}
          {drift && drift.points.length >= 2 && (
            <span
              className="flex items-center gap-1"
              data-testid="drift-trend"
              title={drift.worsening ? '未验证变更占比在上升——结构在漂移' : '未验证占比趋势'}
            >
              漂移
              <Sparkline values={drift.points.map((p) => p.unverifiedRate)} worsening={drift.worsening} />
            </span>
          )}
          {debt && debt.count > 0 && (
            <span
              className="ml-auto shrink-0 px-1.5 py-0.5 rounded bg-danger/15 text-danger"
              data-testid="debt-indicator"
              title="高风险变更被自动放行、从未有人在闸门确认过——没人真正理解的部分"
            >
              理解债 {debt.count}
            </span>
          )}
